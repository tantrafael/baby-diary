...
public class JSON
{
	...
	protected float ParseNumber(char[] json, ref int index)
	{
		...
		index = lastIndex + 1;
		return (float)Double.Parse(new string(numberCharArray), CultureInfo.InvariantCulture);
	}
	...
	{
		// NOTE: Important to check for null first, otherwise unity web player crashes
		if (value == null) {
			builder.Append("null");
			...
			SerializeNumber((float)Convert.ToDouble(value), builder);
